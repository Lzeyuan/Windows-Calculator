...
CalculateNode* InitExpTree(string str) {
	...
	while (str[i] != '\0' || !operationStack.empty()) {
		...
		while (!isdigit(str[i]) && !isOperationChar(str[i])) i++;
		...
		if (!isOperationChar(str[i]) || isNegative(str, i)) {	// 数字部分
			...
			// string(char *x)不支持字符串初始化，可以用string(1, char x)
...
// 运算
BigInter operate(std::string sign, BigInter l, BigInter r) {
	switch (sign[0])
	{
	case '+': {
		return (l + r);
...
// 计算表达式树
 BigInter calculateExpTree(CalculateNode* T) {

	// 操作数左右孩子必为空
	if (!T->ls() && !T->rs()) return BigInter(T->data());

	BigInter l("0");
	BigInter r("0");
	if (T->ls() != NULL) l = l + calculateExpTree(T->ls());
	if (T->rs() != NULL) r = r + calculateExpTree(T->rs());
...
// 计算机函数入口
std::string calculateSolve(std::string op2) {
	std::string ans;
	if (op2 != "0") {
		CalculateNode* T = InitExpTree(op2);
		ans = calculateExpTree(T).ToString();
		destroyExpTree(T);
	}
